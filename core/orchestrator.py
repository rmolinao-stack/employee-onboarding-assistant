...
from common.config import MODEL, DOCS_LIMIT, FAQS_LIMIT, TEMPERATURE, LLM_PROVEEDOR, DIA_ONBOARDING, MODELS_BENCHMARK, PREGUNTAS_PATH
...
Mi objetivo es resolver todas las preguntas que tengas referente al proceso de Onboardig en {data_model.empresa.data["nombre"]}."""
    ...
    while not empleado:
        ...
        if not empleado: 
            ...
            intentos += 1
        if intentos == 3:
            print("Has indicado 3 códigos de empleados invalidos. El chat va a finalizar. Si no conocer tú identificador de empleado debes solicitarlo a RRHH.")
            sys.exit()
...
def procesar_llamada(data_model: DataModel, 
                     ...
                     llm_model: str = MODEL) -> tuple[str, MetricasLlamada]:
    if not vulnerable:
        ...
        if not valido:
            ...
            return [llm_msg, MetricasLlamada(elapsed_ms=None, output_tokens=None, prompt_tokens=None, total_tokens=None)]
        else:
    ...
    respuesta_llm = llamar_llm(prompt, temperature=TEMPERATURE, llm_model=MODEL)
    user_history.append_assistant_message(respuesta_llm[0])
...
def procesar_llamada_json(data_model: DataModel, user_history: UserHistory, docs: list[dict], dia_onboarding: int) -> str:
    ...
    # print (prompt)
    return llamar_llm(prompt, temperature=TEMPERATURE, llm_model=MODEL)[0]
    # return ""
...
def llamar_llm(prompt: str,
    *,
    ...
    if LLM_PROVEEDOR == "GEMINI":
        ...
        return llamar_gemini(prompt=prompt, llm_model=llm_model, temperature=temperature)
    else:
        raise ValueError(f"Proveedor {LLM_PROVEEDOR} de LLM no está soportado.")
    
def conversar_con_llm(user_history: UserHistory,
                      data_model: DataModel, 
                      empl: str, list_msg: list[str], 
                      dia_onboarding: int = DIA_ONBOARDING, 
                      vulnerable: bool = False, 
                      temperature: float = TEMPERATURE,
                      llm_model: str = MODEL) -> tuple[str, MetricasLlamada]:   
    for user_msg in list_msg:
        ...
        respuesta = procesar_llamada(data_model, user_history, user_msg, faqs, docs, vulnerable, temperature=temperature, llm_model=llm_model)
        print(f"|SISTEMA|: {respuesta[0]}")
    return respuesta
...
def ejecutar_benchmark(data_model: DataModel) -> list[FilaBenchmark]:
    preguntas = cargar_json(PREGUNTAS_PATH)
    filas: list[FilaBenchmark] = []

    for p in preguntas:
        pid = p["id"]
        empl_id = p["id_empleado"]
        prompt = p["msg_empleado"]
        for modelo in MODELS_BENCHMARK:
            ts = datetime.now(timezone.utc).isoformat()
            try:
                user_history = inicializar_user_history(data_model, empl_id)
                texto, m = conversar_con_llm(user_history, data_model, empl_id, [prompt], llm_model=modelo, temperature=TEMPERATURE)
                filas.append(
                    FilaBenchmark(
                        timestamp=ts,
                        pregunta_id=pid,
